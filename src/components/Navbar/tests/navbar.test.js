...
import NavBar, { Container, Content, Links, StyledLink } from "../index";

// Mock link component because it needs react router provider
jest.mock("../../Link", () =>
  jest.fn().mockReturnValue(<span>mock link</span>)
);

describe("Navbar Components", () => {
  it("Container has correct default styles", () => {
    const wrapper = mount(<Container theme={theme} />);
    expect(wrapper).toMatchSnapshot();
  });

  it("Content has correct default styles", () => {
    const wrapper = mount(<Content theme={theme} />);
    expect(wrapper).toMatchSnapshot();
  });

  it("Links has correct default styles", () => {
    const wrapper = mount(<Links theme={theme} />);
    expect(wrapper).toMatchSnapshot();
  });

  it("StyledLink has correct default styles", () => {
    const wrapper = mount(<StyledLink theme={theme} />);
    expect(wrapper).toMatchSnapshot();
  });
